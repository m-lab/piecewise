...
db_engine = create_engine("postgresql+psycopg2://{{ database_user }}:@/{{ database+name }}")
...
class ExtraData(Base):
    ...
    client_ip = Column('client_ip', BigInteger)
    {%- for field in form_fields -%}
    {{ field[name] }} = Column('{{ field[name] }}', {{ field[data_type] }})
    {% endfor %}
...
def append_extra_data():
    isp_types = [{%- for key, value in form_fields.isp_user.options.iteritems() -%}
        {{ key }},{% endfor %} 'other']
    connection_types = [{%- for key, value in form_fields.connection_types.options.iteritems() -%}
        {{ key }}{% if not loop.last %},{% endif %}{% endfor %}]
    cost_of_service_types = [{%- for key, value in form_fields.cost_of_service.options.iteritems() -%}
        {{ key }}{% if not loop.last %},{% endif %}{% endfor %}]
...
# need to revisit the validation below to be able to validate fields 
# dynamically entered as global vars
