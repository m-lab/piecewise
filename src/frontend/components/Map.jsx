...
mapboxgl.accessToken =
  'pk.eyJ1IjoicmdhaW5lcyIsImEiOiJjamZuenFmZXIwa2JuMndwZXd1eGQwcTNuIn0.TbNK-TNQxiGUlWFdzEEavw';
...
export default function Map(props) {
  ...
  useEffect(() => {
    ...
    const config = {
      // time periods, as an array of [internalName, displayName]
      time_periods: [
      ...
      // jump-to-state info, an array of [displayName, [lon, lat, zoom]]
      state_centers: [
        ['All States', [38.5266, -96.726486, 3]],
        ['Alabama', [32.806671, -86.79113, 6]],
        ['Alaska', [61.370716, -152.404419, 3]],
        ['Arizona', [33.96, -112.93, 5]],
        ['Arkansas', [34.969704, -92.373123, 5]],
        ['California', [37.56, -120.7, 5]],
        ...
        ['Pennsylvania', [40.7, -78.78, 6]],
        ['Puerto Rico', [18.155, -66.6963, 7]],
        ['Rhode Island', [41.680893, -71.51178, 7]],
        ...
        ['Wyoming', [42.755966, -107.30249], 5],
      ],
      // Array of dynamic layer info, [displayName, layerId, layerSrc, internalLayerName]
      geo_units: [
        ...
        [
          'US Census Tracts',
          'census_tracts',
          'mapbox://newamerica.usbb_tract',
          'usbb_tract',
        ],
        [
          'State Senate Districts',
          'state_senate',
          'mapbox://newamerica.usbb_state_senate',
          'usbb_state_senate',
        ],
        [
          'State House Districts',
          'state_house',
          'mapbox://newamerica.usbb_state_house',
          'usbb_state_house',
        ],
      ],
      // Array of display styling info, {displayName, internalName, mapboxGlStyleFn, stops, popupFn}
      ramps: [
        mkRamp({
          ...
          mapboxGlStyleFn(timePeriod, stops) {
            return [
              ...
              [
                'interpolate',
                ['linear'],
                ['get', `ml_download_Mbps_${timePeriod}`],
                ...stops,
              ],
              '#808080',
            ];
          ...
          mapboxGlStyleFn(timePeriod, stops) {
            return [
              ...
              [
                'interpolate',
                ['linear'],
                ['get', `ml_upload_Mbps_${timePeriod}`],
                ...stops,
              ],
              '#808080',
            ];
          ...
          mapboxGlStyleFn(timePeriod, stops) {
            return [
              ...
              [
                'interpolate',
                ['linear'],
                ['get', `ml_dl_count_tests_${timePeriod}`],
                ...stops,
              ],
              '#808080',
            ];
          ...
          mapboxGlStyleFn(timePeriod, stops) {
            return [
              ...
              [
                'interpolate',
                ['linear'],
                ['get', `ml_ul_count_tests_${timePeriod}`],
                ...stops,
              ],
              '#808080',
            ];
          ...
          mapboxGlStyleFn(timePeriod, stops) {
            return [
              ...
              [
                'interpolate',
                ['linear'],
                ['get', `ml_dl_count_ips_${timePeriod}`],
                ...stops,
              ],
              '#808080',
            ];
          ...
          mapboxGlStyleFn(timePeriod, stops) {
            return [
              ...
              [
                'interpolate',
                ['linear'],
                ['get', `ml_ul_count_ips_${timePeriod}`],
                ...stops,
              ],
              '#808080',
            ];
            ...
            mapboxGlStyleFn(timePeriod, stops) {
              return [
                ...
                [
                  'interpolate',
                  ['linear'],
                  ['get', `fcc_advertised_down_${timePeriod}`],
                  ...stops,
                ],
                '#808080',
              ];
          ...
          mapboxGlStyleFn(timePeriod, stops) {
            return [
              ...
              [
                'interpolate',
                ['linear'],
                ['get', `fcc_advertised_up_${timePeriod}`],
                ...stops,
              ],
              '#808080',
            ];
    ...
    function mlabPopupFn(props) {
      ...
      const formatRow = ([time, dl_key, ul_key]) =>
        `<div class="report-date">${time}</div><div class="report-value">${rate(
          dl_key,
        )}</div>` + `<div class="report-value">${rate(ul_key)}</div>`;

      var dataset =
        measureSelect.current.options[measureSelect.current.selectedIndex]
          .parentNode.id;

      return (
        '<h2>' +
        (props['county_name'] || props['tract_name'] || props['name']) +
        '</h2>' +
        '<em>Source:' +
        dataset +
        '</em>' +
    ...
    function fccPopupFn(props) {
      ...
      const rows = [
        [
          'Jan 1-4, 2020',
          'fcc_advertised_down_dec_2014',
          'fcc_advertised_up_dec_2014',
        ],
        [
          'Jan 5-11, 2020',
          'fcc_advertised_down_jun_2015',
          'fcc_advertised_up_jun_2015',
        ],
        [
          'Jan 12-18',
          'fcc_advertised_down_dec_2015',
          'fcc_advertised_up_dec_2015',
        ],
        [
          'Jan 19-25, 2020',
          'fcc_advertised_down_jun_2016',
          'fcc_advertised_up_jun_2016',
        ],
        [
          'Jan 26-Feb 1, 2020',
          'fcc_advertised_down_dec_2016',
          'fcc_advertised_up_dec_2016',
        ],
        [
          'Feb 2-8, 2020',
          'fcc_advertised_down_jun_2017',
          'fcc_advertised_up_jun_2017',
        ],
      ];
      const formatRow = ([time, dl_key, ul_key]) =>
        `<div class="report-date">${time}</div><div class="report-value">${rate(
    ...
    const initializeMap = ({ setMap, mapContainer }) => {
      ...
      function updateFragment() {
        ...
        url.hash = `#${geoSelect.current.value}/${timeperiod}/${
          measureSelect.current.value
        }/${x.toFixed(2)}/${y.toFixed(2)}/${z.toFixed(2)}`;
        window.history.replaceState(undefined, 'USBB', url.toString());
      }

      function configureMap() {
        const ramp = config.ramps.find(
          ramp => ramp.internalName == measureSelect.current.value,
        );

        // Apply time constraint and clamp value.
        const maxTimeIndex = config.time_periods.findIndex(
          period => period[0] == ramp.maxTime,
        );
        console.log(ramp.maxTime, maxTimeIndex, config.time_periods.length);
        timeSlider.current.max =
          maxTimeIndex == -1 ? config.time_periods.length - 1 : maxTimeIndex;
        timeSlider.current.value = Math.min(
          timeSlider.current.max,
          timeSlider.current.value,
        );
        ...
        for (const layer of config.geo_units) {
          map.setLayoutProperty(
            layer[1],
            'visibility',
            layer[1] == geoSelect.current.value ? 'visible' : 'none',
          );
        ...
        legend.current.style.background = `linear-gradient(to right, ${cssRamp})`;
        const cssLabels = breaks
          .filter((_, idx) => !(idx % 2))
          .filter(b => Number.isFinite(b));
        legendlabel.current.innerHTML = cssLabels
          .map(l => `<div class="label">${l}</div>`)
          .join('');
      ...
      map.on('load', () => {
        ...
        if (fragment) {
          ...
          const geo = check(parts[0], v =>
            config.geo_units.some(u => u[1] == v),
          );
          const timeperiod = check(parts[1], v =>
            config.time_periods.some(x => x[0] == v),
          );
          const measure = check(parts[2], v =>
            config.ramps.some(r => r.internalName == v),
          );
        ...
        [measureSelect, geoSelect, timeSlider].forEach(el =>
        ...
        stateSelector.current.addEventListener('input', el => {
          const state = config.state_centers.find(
            row => row[0] == stateSelector.current.value,
          );
          ...
          <div className="map-overlay top">
            <div className="map-overlay-inner">
              ...
              <label htmlFor="measurement">
                Data{' '}
                <abbr title="Choose the measurement to show on the map.">
                  ?
                </abbr>
              </label>
              <select
              ...
              <div style={{ width: '100%' }}>
                <details>
                  ...
                  Cum ceteris in veneratione tui montes, nascetur mus. Morbi
                  fringilla convallis sapien, id pulvinar odio volutpat. Tityre,
                  tu patulae recubans sub tegmine fagi dolor. Ambitioni dedisse
                  scripsisse iudicaretur.
                </details>
              </div>
              <label htmlFor="geo">
                Area{' '}
                <abbr title="Choose the detail level of the places on the map.">
                  ?
                </abbr>
              </label>
              <select
                ...
                ref={geoSelect}
                onChange={_updateGeo}
                defaultValue={geography.value}
                className="mb-2"
              />
              <label htmlFor="slider">
                Time Period{' '}
                <abbr title="Choose the time period to show on the map.">
                  ?
                </abbr>
              </label>
              <label id="time_period" ref={timePeriodLabel} />
              <input
                id="slider"
                type="range"
                ref={timeSlider}
                className="mb-2"
              />
              <label htmlFor="state_select">
                Zoom to State{' '}
                <abbr title="Center the map on a specific state.">?</abbr>
              </label>
              <select
                id="state_select"
                name="state_select"
              ...
              <div className="session">
                ...
                <div id="noresultlabel" className="row labels">
                  <div className="label label-no-results">
                    No tests available in time period
                  </div>
                </div>
              </div>
            </div>
