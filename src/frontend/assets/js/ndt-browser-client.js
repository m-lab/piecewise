...
NDTjs.prototype.makeLoginMessage = function(desiredTests) {
  ...
  var i,
    loginMessage =
      'XXX { "msg": "v3.5.5", "tests": "' + (desiredTests | 16) + '" }',
...
NDTjs.prototype.parseNdtMessage = function(buffer) {
  var i,
    ...
    message = String.fromCharCode.apply(null, new Uint8Array(buffer.slice(3)));
  for (i = 0; i < 3; i += 1) {
 ...
 */
NDTjs.prototype.createWebsocket = function(
  serverProtocol,
  serverAddress,
  serverPort,
  urlPath,
  ...
  var createdWebsocket = new WebSocket(
    serverProtocol + '://' + serverAddress + ':' + serverPort + urlPath,
    protocol,
  );
 ...
 * @returns {Websocket} The WebSocket we created;
 */
NDTjs.prototype.createBulkWebsocket = function(
  serverProtocol,
  serverAddress,
  serverPort,
  urlPath,
  ...
  keepSendingData = function() {
    ...
    if (connectionOpen) {
      ...
      if (that.updateInterval && currentTime > testStart + nextCallback) {
        that.results.c2sRate =
          (8 * (totalSent - testConnection.bufferedAmount)) /
          1000 /
  ...
  return function(messageType, messageContent) {
    ...
    if (
      ...
      testConnection.onmessage = function(response) {
        ...
        currentTime = Date.now() / 1000.0;
        if (that.updateInterval && currentTime > testStart + nextCallback) {
          that.results.s2cRate =
            (8 * receivedBytes) / 1000 / (currentTime - testStart);
    ...
    if (state === 'WAIT_FOR_FIRST_TEST_MSG' && messageType === that.TEST_MSG) {
      ...
      console.log(
        'S2C rate calculated by server: ' + messageContent.ThroughputValue,
      );
      ndtSocket.send(
        that.makeNdtMessage(that.TEST_MSG, String(that.results.s2cRate)),
      );
      return false;
    }
    if (
...
NDTjs.prototype.ndtMetaTest = function(ndtSocket) {
  ...
  return function(messageType, messageContent) {
    ...
    if (state === 'WAIT_FOR_TEST_START' && messageType === that.TEST_START) {
      ...
      ndtSocket.send(
        that.makeNdtMessage(that.TEST_MSG, 'client.os.name:NDTjsPiecewise'),
      );
    ...
    errorMessage =
      'Bad state and message combo for META test: ' +
      state +
      ', ' +
      messageType +
      ', ' +
...
NDTjs.prototype.startTest = function() {
  ...
  ndtSocket = this.createWebsocket(
    this.serverProtocol,
    this.server,
    this.serverPort,
    this.serverPath,
    ...
    if (state === 'LOGIN_SENT') {
      ...
      if (messageType === that.SRV_QUEUE) {
        if (messageContent.msg === '9990') {
          ...
          throw that.TestFailureException(
            'Server terminated test ' + 'with SRV_QUEUE 9977',
          );
        }
        ...
        errorMessage =
          'Expected type 1 (SRV_QUEUE) or 2 (MSG_LOGIN)' +
          'but got ' +
          messageType +
          ' (' +
          that.NDT_MESSAGES[messageType] +
      ...
      errorMessage =
        'No handler for message ' + messageType + ' in state ' + state;
      throw that.TestFailureException(errorMessage);
    }
  };
